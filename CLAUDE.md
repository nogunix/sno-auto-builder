...
| `99-destroy-all.yml` | localhost | Removes the MCP pod/secret, `tofu destroy`, manual `virsh undefine` fallbacks, removes `sno_base_dir`, cleans up this cluster's nginx backend (rebuilds config if other clusters remain, otherwise tears nginx down), removes per-cluster dnsmasq DNS config, and all three `/etc/hosts` entries |
...
- `nginx-sno-stream.conf.j2` → `/etc/nginx/stream.d/sno.conf` — SNI-based stream proxy for ports 80/443/6443, supports multiple clusters via `ssl_preread` routing (rendered by `03`)
